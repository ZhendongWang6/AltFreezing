...
def get_clips_uniform(path, count, clip_size):
    ...
    start_indices = np.linspace(0, final_start, count, endpoint=True, dtype=int)
...
def get_bbox(detect_res):
    ...
    all_face_bboxs = np.array(all_face_bboxs).astype(int)
...
def pre_crop(clips, detect_res):
    ...
    w = box[2] - box[0]
    h = box[3] - box[1]
    x1, y1, x2, y2 = scale_box(box, 1.5, 1.2 if w > 2 * h else 1.5, clips[0].shape[0], clips[0].shape[1])

...
def estimiate_batch_transform(all_src_pts, tgt_pts=std_points_317):
    tgt_pts = np.repeat(tgt_pts[None, ...], len(all_src_pts), 0).reshape(-1, 2) # 复制tgt点的坐标
    src_pts = np.array(all_src_pts).reshape(-1, 2)
    tfm, trans = get_similarity_transform_for_cv2(src_pts, tgt_pts)
    return tfm, trans

def estimiate_batch_transform_selfblend(all_src_pts, all_tgt_pts):
    tgt_pts = np.array(all_tgt_pts).reshape(-1, 2)
    src_pts = np.array(all_src_pts).reshape(-1, 2)
    tfm, trans = get_similarity_transform_for_cv2(src_pts, tgt_pts)
    return tfm, trans

...
class ModelBase(nn.Module):
    ...
    def load(self, fullpath=None, epoch=-1, pretrained=None):
        ...
        try:
            ...
            if unfind_key:
                logger.warn("{} are in model, but not found in chekppint loaded for {}".format(unfind_key, self.name))
            if unmatch_key:
                logger.warn("{} have unmatching shape between checkpoint&model for {}".format(unmatch_key, self.name))
            self.network.load_state_dict(param_dict)
            logger.info("load weights from {}".format(fullpath))
    ...
    def make_distributed(self, distributed=True):
        ...
        if dist.is_initialized() and distributed:
            ...
            #                 )
            self._warped_network = torch.nn.parallel.DistributedDataParallel(
                self._warped_network,
                device_ids=[torch.cuda.current_device()],
                output_device=torch.cuda.current_device(),
    ...
    def find_last(self, epoch=-1, model_dir=None):
        ...
        if epoch == -1:
            ...
            raise RuntimeError("epoch {} is bigger than all exist checkpoints".format(epoch))
        else:

...
class SpatialTemporal_Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ### v1: 取每一帧的cls token作为temporal transformer的输入
...
class SpatialTemporal_Transformer_v2(nn.Module):
    def __init__(self, image_size, patch_size, num_classes, num_frames, dim = 192, depth = 4, heads = 3, pool = 'cls', in_channels = 3, dim_head = 64, dropout = 0.,
                 emb_dropout = 0., scale_dim = 4, ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        ### v1: 取每一帧的cls token作为temporal transformer的输入
        # x = rearrange(x[:, 0], '(b t) ... -> b t ...', b=b)
        ### v2: 取每一帧的spatial tokens的mean作为temporal transformer的输入
        # x = rearrange(x[:, 1:, :], '(b t) ... -> b t ...', b=b)
        x = rearrange(x, '(b t) ... -> b t ...', b=b)
        x = x.mean(dim = 2) ## (b,t,n,d) -> (b,t,d)
